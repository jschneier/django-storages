...
class AzureStorage(BaseStorage):
    ...
    def service(self):
        if self._service is None:
            ...
            self._service = self._blob_service(
                custom_domain=custom_domain, connection_string=self.connection_string
            )
    ...
    def exists(self, name):
        return self.service.exists(
            self.azure_container, self._get_valid_path(name), timeout=self.timeout
        )
    ...
    def list_all(self, path=""):
        ...
        return [
            ...
            for blob in self.service.list_blobs(
                self.azure_container, prefix=path, timeout=self.timeout
            )
        ]
