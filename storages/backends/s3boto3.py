...
if boto3_version_info[:2] < (1, 1):
    raise ImproperlyConfigured("The installed Boto3 library must be 1.1.0 or "
                               "higher.\nSee https://github.com/boto/boto3")
...
class S3Boto3StorageFile(File):
    ...
    def __init__(self, name, mode, storage, buffer_size=None):
        ...
        # TODO(mbarrien): Is this explicitly necessary? Done to emulate old
        # behavior, but I think the code would misbehave already if
        # self.obj is None.
        if 'w' not in mode:
            try:
                self.obj.load()
...
class S3Boto3Storage(Storage):
    ...
    def _save(self, name, content):
        ...
        if self.preload_metadata:
            obj.reload()  # TODO(mbarrien): This is done to update last_modified
        return cleaned_name
