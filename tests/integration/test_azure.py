...
class AzureStorageTest(TestCase):
    ...
    def test_url(self):
        self.assertTrue(self.storage.url("my_file.txt").endswith("/test/my_file.txt"))
    ...
    def test_url_unsafe_chars(self):
        ...
        self.assertTrue(self.storage.url(name).endswith(expected))
        # has some query-string
        self.storage.expiration_secs = 360
...
class AzureStorageDjangoTest(TestCase):
    ...
    def test_template_static_file(self):
        t = Template("{% load static from staticfiles %}" '{% static "foo.txt" %}')
        self.assertEqual(
    ...
    def test_template_media_file(self):
        t = Template("{{ file_url }}")
        rendered = t.render(
            Context({"file_url": default_storage.url("foo.txt")})
        ).strip()
        self.assertTrue(
    ...
    def test_form(self):
        files = {
            "foo_file": SimpleUploadedFile(
                name="1234.pdf", content=b"foo content", content_type="application/pdf"
            )
        }
        form = FooFileForm(data={}, files=files)
        self.assertTrue(form.is_valid())
        name = default_storage.save("foo.pdf", form.cleaned_data["foo_file"])
    ...
    def test_model_form(self):
        files = {
            "foo_file": SimpleUploadedFile(
                name="foo.pdf", content=b"foo content", content_type="application/pdf"
            )
        }
    ...
    def test_name_clean_issue_609(self):
        ...
        simple_file.foo_file = SimpleUploadedFile(
            name="foo%?:;~bar.txt", content=b"foo content"
        )
        simple_file.save()
    ...
    def test_name_clean_issue_609_with_special_chars(self):
        ...
        name = "foo%?:;~bar.txt"
        simple_file = SimpleFileModel()
        simple_file.foo_file = SimpleUploadedFile(name=name, content=b"foo content")
        simple_file.save()
        self.assertEqual(simple_file.foo_file.name, "foo_uploads/{}".format(name))
        self.assertTrue(
